...
现在任何收费的都是盗版软件。
下载安装流程比较复杂，可以参考这篇这篇知乎文章：\underline{{\href{https://zhuanlan.zhihu.com/p/1974155096729331109}{https://zhuanlan.zhihu.com/p/1974155096729331109}}}。

另外，要下载系统的镜像文件，在虚拟机软件VMware中创建虚拟机时需要用到。镜像软件可在发行版官网或者开源镜像网站找到，比如Ubuntu24.04.2 LTS版本，
可以在清华大学开源软件镜像站（\underline{{\href{https://mirrors.tuna.tsinghua.edu.cn/ubuntu-releases/24.04.2/}{https://mirrors.tuna.tsinghua.edu.cn/ubuntu-releases/24.04.2/}}}）下载，
打开后选择\code{ubuntu-24.04.2-desktop-amd64.iso}下载即可。这里的\code{desktop}表示这个系统包含了桌面环境，\code{amd64}表示这个系统适用于一般64位的计算机（绝大多数安装Windows系统的电脑都能用这个版本）。

如果是M芯片的Mac，则需要使用包含\code{arm64}版本的镜像文件，\code{arm64}表示这个系统适用于ARM架构的计算机（极小部分Windows系统的电脑是这个架构）。
注意，\code{arm64}版本的镜像文件不包含桌面环境，安装后为命令行界面，如需桌面环境，可在安装完成后运行\code{sudo apt install ubuntu-desktop}命令安装桌面环境。



都下载完成后，虚拟机具体安装流程可参考这一篇知乎文章：\underline{{\href{https://zhuanlan.zhihu.com/p/695848883}{https://zhuanlan.zhihu.com/p/695848883}}}。以上两篇文章建议先通读一遍，了解整体流程，再逐步安装。
上面提到的VMware Workstation Pro和VMware Fusion还有Ubuntu24.04 LTS镜像文件，我打包了一个压缩包，放在了夸克网盘（\underline{{\href{https://pan.quark.cn/s/bfc066dbd66a}{https://pan.quark.cn/s/bfc066dbd66a}}}）可以直接下载解压后使用。
也可以加入学习通课程（邀请码\code{91364739}），在课程资料中获取（下载可能会快一些），由于学习通限制，所有文件打包成了压缩包，请解压使用。
邀请码2026年8月7日前有效，我会不断更新在此文档并发布在项目主页上。

安装之后，在桌面同时按\code{ctrl+Alt+T}呼出终端。运行如下指令（需要输入的是\$之后的部分），设置\code{root}用户密码：
...
在Linux中，通常不建议直接使用\code{root}用户进行日常操作，而是通过\code{sudo}临时提升权限。这里设置\code{root}密码仅用于特殊情况。
...
具体安装细节可参考这篇知乎文章：\underline{{\href{https://zhuanlan.zhihu.com/p/76032647}{https://zhuanlan.zhihu.com/p/76032647}}}。安装相较于虚拟机简单很多，
但后期卸载残留难以处理，且性能较虚拟机差，建议有条件的同学优先使用虚拟机安装Linux。
...
\code{/usr}：存放用户空间程序和库文件，大多数系统软件安装在此目录下；
...
\code{cp}命令有\code{-r}选项（recursive），与\code{mkdir -p}类似，表示递归复制目录及其内容。
...
一个简单的例子：\code{ls}能列出当前目录下的文件和子目录，\code{grep}命令（后面会介绍）可以用于过滤这些文件和子目录，我们可以将这两个命令组合起来，查看当前目录下特定的文件：
\begin{envcode}{console}{Bash}
user1@host:~$ ls -l | grep .txt
\end{envcode}
上述命令会将\code{ls}的输出作为\code{grep}的输入，\code{grep}会过滤出包含\code{.txt}的行，即当前目录下含\code{.txt}的文件的详细信息。
...
Ubuntu中\code{useradd}默认不创建用户主目录，需加-m参数\code{sudo useradd -m user2}，或使用\code{adduser}命令\code{sudo adduser user2}，它会自动创建用户主目录并设置相关权限。
\end{zy}
...
\item \code{/匹配的文本/d}：删除匹配模式的行
\item \code{/匹配的文本/a}：在匹配模式的行后添加文本
\item \code{/匹配的文本/i}：在匹配模式的行前添加文本
\end{itemize}
...
Vim有官方的教程，部分版本有中文版，在终端运行：
\begin{envcode}{console}{Bash}
user1@host:~$ vimtutor zh
\end{envcode}
若运行失败，则说明没有中文版，可以直接运行\code{vimtutor}命令。可以自行在其中练习，熟悉Vim的基本操作。
...
用\code{:}、\code{/}、\code{?}等进入底线命令模式。在非一般模式下，按\code{Esc}键可以返回一般模式。
...
\subsubsection{Vim相关背景}
最早程序员们使用的是叫做ex的行编辑器，vi是其中的一个命令，提供可视化界面，后由于太常用，被单独拿出来成为一个编辑器。
但由于版权原因，Vi最早只能在Unix系统中使用。Bram Moolenaar就购买了一台Amiga电脑。因为习惯使用Vi进行编辑，所以他在Amiga上寻找了类似Vi的程序。
但他尝试了Vi的许多“克隆”版本，但没有一个符合他的要求，于是他决定自己编写一个新的编辑器，这就是Vim（Vi IMproved，Vi的改进版）。

Vim由于其强大的功能、丰富的插件和高度可定制性，迅速在程序员中流行起来，并成为了许多开发者的首选编辑器，
以至于到现在，在终端中运行Vi，可能实际上就是运行Vim的，这是因为在许多系统中，vi命令已经被默认链接给了Vim。

Bram编写、分发Vim都是完全免费的，尽管如此，还是有许多用户自愿支持他，但Bram本人并不想要这些捐款，
于是他利用“非洲孤儿学校志愿者”的身份来接受捐款，捐款会被用来帮助非洲的孤儿接受教育。
由于Bram早年在非洲工作过，所以他对非洲的孤儿学校非常熟悉，并且一直在为这些学校提供支持和帮助，这些捐款的利用率非常高（高达99.5\%），几乎全部都被用来帮助非洲的孤儿接受教育。

这也让Vim成为了第一款“慈善软件”：软件本身免费，如果你觉得它对你有很大的帮助，愿意为之付费，可以捐款给非洲乌干达的儿童。
现在在命令行输入并运行\code{vim}，就会打开Vim编辑器并显示软件信息，其中就有一句话“帮助乌干达的可怜儿童！”

直到2023年7月，Bram还在更新维护Vim，随后便失去了消息。
2023年8月5日，Bram Moolenaar的家人发布讣告，为全世界开发者带来了一个悲伤的讯息——我们怀着沉重的心情通知您，在过去几周内Bram Moolenaar因病情迅速恶化，于2023年8月3日离世，享年62岁。

如今斯人已逝，用Vim的语言说，他这一生已经\code{:wq}，但Vim的开发还在继续，Vim的社区也在继续维护和发展这个伟大的编辑器，相关基金会也在继续接收捐款帮助非洲儿童。

\begin{envcode}{console}{Vim}
:wq
\end{envcode}
